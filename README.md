...
### **For Developers / Operators Building Production Systems**
...
#### **2. Option A: Static Demo (No Backend)**
```bash
...
#### **3. Option B: Live Excel Backend (Full Features)**
```bash
# Run the Python anomaly detection server
python3 server.py

# Server runs on http://localhost:8000
# Watches data/lithohub_assets.xlsx every 30s
# Computes anomalies via /api/anomalies/{asset_id}
```

#### **4. Adapt to Your Data**

**Replace `data/assets.json` with your own:**
```json
[
  {
    "id": "well-001",
    "name": "Your Well Name",
    "type": "Geothermal Well",  // or Oil Well, Gas Well, Pipeline, Processing Facility
    "location": "Region",
    "coordinates": { "latitude": -7.0, "longitude": 107.0 },
    "rockType": "Sandstone (Oligocene)",
    "depth": 2800,
    "operatingsSince": "2015-06-15",
    "readings": [
      {
        "timestamp": "2026-05-01T08:00:00Z",
        "whp": 42.5,        // Wellhead Pressure (bar)
        "temperature": 245, // °C
        "chloride": 171     // ppm
      },
      // ... 89 more readings (90-day rolling window)
    ]
  }
]
```
...
**Fields required:**
- `id`, `name`, `type`, `coordinates` (latitude, longitude)
- `readings` array with at least 90 points, each: `timestamp`, `whp`, `temperature`, `chloride`
- Optional: `rockType`, `depth`, `operatingsSince` (improve geological context)

**Types supported:**
- `Geothermal Well` (200°C normal, 310°C warning, 350°C critical)
- `Oil Well` (60°C normal, 85°C warning, 120°C critical)
- `Gas Well` (70°C normal, 100°C warning, 135°C critical)
- `Pipeline` (50°C normal, 75°C warning, 100°C critical)
- `Processing Facility` (60°C normal, 85°C warning, 110°C critical)

---

## How Anomaly Detection Works

### **Threshold-Based (Transparent, Auditable)**

**1. Statistical Baseline (90 days)**
```
baseline_mean = mean(last_90_readings)
baseline_std  = stdev(last_90_readings)
threshold     = baseline_mean ± 1.5 × baseline_std
```

**2. Real-Time Comparison**
```
current_reading = latest_sensor_value
if |current_reading - baseline_mean| > 1.5 × baseline_std:
  FLAG AS ANOMALY
```

**3. Geological Context**
- Asset type thresholds override statistical thresholds (e.g., 245°C on geothermal = normal even if >1.5σ)
- Rock type modifiers adjust risk interpretation
- Depth + pressure gradients inform root cause

**4. Operator Decision Support**
Instead of "Anomaly Detected," users see:
```
⚠️ Chloride Concentration Spike
Current: 315 ppm | Baseline: 171 ppm (+84%)
Likely Cause: Casing integrity compromise OR cold water infiltration
Recommended Action: Pull production tubing for inspection within 30 days
Economic Impact: If casing failed, production loss = $150K/day
```
